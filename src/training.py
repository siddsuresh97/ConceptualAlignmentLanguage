...
def wandb_init(epochs, lr, train_mode, batch_size, model_number,data_set):
  run = wandb.init(project="ConceptualAlignmentLanguage", entity="psych-711", reinit=True)
  wandb.config = {
    "learning_rate": lr,
    "epochs": epochs,
...
def train(save_dir, num_models, epochs, num_classes, batch_size,
             lr, latent_dims, model_architecture, device, data_dir):
  if os.path.isdir(os.path.join(save_dir, model_architecture)):
    pass
  else:
    os.mkdir(os.path.join(save_dir,model_architecture))
  ...
  for data_set in ['base','overlap_50','overlap_20']:
    for i in tqdm(range(3)):
      ...
      for model_number in range(num_models):
        if model_architecture == 'ConvAutoencoder':
            model = ConvAutoencoder.ConvAutoencoder(latent_dims, num_classes, device).to(device) # GPU)
        else:
            print('model name invalid')
        run = wandb_init(epochs, lr, train_mode, batch_size, model_number,data_set)
        if data_set=='base':
